...
const MyDoc = ({ subOrderData, getURL }) => {
  ...
  return (
    <>
      <Document>
        ...
        >
          ...
          <View style={styles.table}>
            ...
            {subOrderData.length > 0 &&
              subOrderData?.[0]?.OrderItems?.map((el, index) => (
                <View>
          ...
          {subOrderData.length > 0 && (
            <View>
              <View style={styles.totalPrice}>
                <Text><IntlMessage id="Order.orderDeatils.printRecipts.totalAmount"/> - {subOrderData?.[0]?.sub_total}</Text>
                <Text>
                <IntlMessage id="profileSection.vat"/> - {subOrderData?.[0]?.vat}
                </Text>
                <Text>
                <IntlMessage id="profileSection.serviceCharge"/>   -{" "}
                  {subOrderData?.[0]?.service_tax}
                </Text>
                <Text>
                <IntlMessage id="userManagement.rolePermission.discount"/>   -{" "}
                  {subOrderData?.[0]?.discount}
                </Text>
              </View>

              <View style={styles.grandTotal}>
                <Text><IntlMessage id="Order.orderDeatils.printRecipts.totalAmount"/> - {subOrderData?.[0]?.total}</Text>
              </View>
            </View>
          )}
        </Page>
            ...
            >
              ...
              {subOrderData.length > 0 && (
                <View style={styles.PriceSection}>
                  <View style={styles.totalPrice}>
                    <Text>Total Amount - {subOrderData?.[0]?.sub_total}</Text>
                    <Text>
                      Vat {RestaurentDetails?.vat}% - {subOrderData?.[0]?.vat}
                    </Text>
                    <Text>
                  ...
                  <View style={styles.grandTotal}>
                    <Text>Total Amount - {subOrderData?.[0]?.total}</Text>
                  </View>
                </View>
              )}
            </Page>
      ...
      >
        {({ blob, url, loading, error }) => getURL(blob, url, loading, error)}
      </PDFDownloadLink>
    </>
  );
};

export default function PrintReceipt({ open, handleClose, subOrderData }) {
